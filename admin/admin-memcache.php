...
class WPTRUECACHE_WPMemcachedManager {
	//the servers we have setup
	var $memcached_servers = array();

	//Simple constructor, create top level menu
	function WPTRUECACHE_WPMemcachedManager() {
		//add_action( 'admin_menu', array(&$this, 'wpmm_menu' ));
		//$wpmm_options = get_option( 'wp_memcached_manager' );
		$this->memcached_servers = get_memcache_server_list(); // ( is_serialized( $wpmm_options )) ? unserialize( $wpmm_options ) : $wpmm_options;
	...
	function wpmm_add_all_servers(){
		if ( ! empty( $this->memcached_servers )) {
			...
			foreach( $this->memcached_servers as $memcached_server ) {
				$this->wpmm_add_server( $id++, $memcached_server['host'], $memcached_server['port'] );
			}
		}
	...
	function wpmm_build_server_select( $title_shown, $selected_server = "" ){
		...
		foreach( $this->memcached_servers as $key => $value ) {
			if ( $selected_server == $this->memcached_servers[$key]['server_id'] )
				$selected = "selected";
			echo "<option value='" . $i++ . "' " . $selected . " >" . $value['host'] . ":" . $value['port'] . "</option>";
			$selected = "";
		}
	}
}
...
class WPTRUECACHE_WPMemcachedServer extends Memcache {

	var $memcached_server_host;
	var $memcached_server_port;
	var $memcached_server_id;  //format: host-port

	function WPTRUECACHE_WPMemcachedServer( $memcached_server_id, $memcached_server_host, $memcached_server_port ) {
...
//Headings, etc
function wptruecache_wpmm_page_start(){
	global $wpmm_manager;
	?>
		<div class="wrap"><h2>Memcached Manager</h2>
	<?php
...
function wptruecache_wpmm_main_page() {
	global $wpmm_manager;

	wptruecache_wpmm_page_start();

	if ( ! empty( $wpmm_manager->memcached_servers )) { ?>
			<form action="?page=memcache" method="post">
		...
		wp_nonce_field('wptruecache-wpmm-select-server-nonce'); ?>
		...
		if ( "Manage Server" == $_POST['manage-action'] ) {
			...
			if (check_admin_referer('wptruecache-wpmm-select-server-nonce')) {
				//derive the server info from the server_id
				$strs = explode('-', $_POST['memcache_server_id'] );
				$host = $wpmm_manager->memcached_servers[$_POST['memcache_server_id']]['host']; // $strs[0];
				$port = $wpmm_manager->memcached_servers[$_POST['memcache_server_id']]['port']; //$strs[1];

				$single_server = new WPTRUECACHE_WPMemcachedServer( $memcached_server['server_id'], $host, $port );
				?>
					<p/>
					<table class="widefat">
					<thead>
				...
				?>
					...
					<div class="alignleft actions">
					<form action="?page=memcache" method="post">
					<?php	wp_nonce_field('wptruecache-wpmm-manage-action-nonce'); ?>
		...
		elseif ( "Flush Memcache" == $_POST['manage-action'] ) {
			if (check_admin_referer('wptruecache-wpmm-manage-action-nonce')) {
				...
				?>	<p/>
					<form action="?page=memcache" method="post">
					<?php	wp_nonce_field('wptruecache-wpmm-confirm-action-nonce'); ?>
		...
		elseif ( "Flush Confirm" == $_POST['manage-action'] ) {
			if (check_admin_referer('wptruecache-wpmm-confirm-action-nonce')) {
				
				$strs = explode('-', $_POST['memcache_server_id'] );
				$host = $wpmm_manager->memcached_servers[$_POST['memcache_server_id']]['host']; // $strs[0];
				$port = $wpmm_manager->memcached_servers[$_POST['memcache_server_id']]['port']; //$strs[1];
				
				$single_server = new WPTRUECACHE_WPMemcachedServer( $_POST['memcache_server_id'], $host, $port );
		...
		elseif ( "View Confirm" == $_POST['manage-action'] ) {
			...
			<?php		
				...
				echo "<div id='notice' class='updated fade'>No Memcache connections exist</div>";
			}
		}				
	}	

	wptruecache_wpmm_page_end();
}

function wptruecache_wpmm_edit_page(){
	global $wpmm_manager;
	...
	if ($_POST['server_host']) {
		if (check_admin_referer('wptruecache-wpmm-options-add-server-nonce')) {
	...
	if ( $_GET['delete'] ) {
		if ( check_admin_referer( 'wptruecache-wpmm-delete-server-nonce' )) {
	...
	wp_nonce_field('wptruecache-wpmm-options-add-server-nonce');
	...
	if ( ! empty( $wpmm_manager->memcached_servers )) {
		foreach( $wpmm_manager->memcached_servers as $memcached_server ) {
			...
			$single_server = new WPTRUECACHE_WPMemcachedServer( $memcached_server['server_id'], $memcached_server['server_host'], $memcached_server['server_port'] );
			print('<tr>');
			...
			$delete_server_link = wp_nonce_url($delete_server_link, 'wptruecache-wpmm-delete-server-nonce');
...
function wptruecache_wpmm_page_end(){
	echo "</div>";
}

if ( class_exists('WPTRUECACHE_WPMemcachedManager') ) {
	$wpmm_manager = new WPTRUECACHE_WPMemcachedManager();
}
